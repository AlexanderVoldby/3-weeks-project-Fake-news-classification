...
ConfusionMatrixDisplay.from_estimator(logreg, X_test, y_test, display_labels=["wiki", "wiki"])

# Twitter-trained model
X_train, X_test, y_train, y_test = train_test_split(twitter_art_matrix, y_corr, random_state=0)
logreg = LogisticRegression(max_iter=1000).fit(X_train, y_train)
ConfusionMatrixDisplay.from_estimator(logreg, X_test, y_test, display_labels=["twitter", "twitter"])

# Selftrained model
X_train, X_test, y_train, y_test = train_test_split(selftrained_art_matrix, y_corr, random_state=0)
logreg = LogisticRegression(max_iter=1000).fit(X_train, y_train)
ConfusionMatrixDisplay.from_estimator(logreg, X_test, y_test, display_labels=["selftrained", "selftrained"])
...
ConfusionMatrixDisplay.from_estimator(classifier1, article_representation(
    unseen_embedding_corpus, wikipedia_embeddings_index, 100), unseen_embedding_y, display_labels=["wiki_real", "wiki_fake"])
...
ConfusionMatrixDisplay.from_estimator(classifier1, article_representation(
    unseen_embedding_corpus, twitter_embeddings_index, 100), unseen_embedding_y, display_labels=["twitter_real", "twitter_fake"])
...
ConfusionMatrixDisplay.from_estimator(classifier3, unseen_art_matrix, unseen_embedding_y,
                       display_labels=["selftrained_real", "selftrained_fake"])
