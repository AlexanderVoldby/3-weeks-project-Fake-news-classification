...
ConfusionMatrixDisplay.from_estimator(logreg, X_test, y_test, display_labels=["unfiltered", "unfiltered"])

X_train, X_test, y_train, y_test = train_test_split(X2, bow_y, random_state=0)
print(bow_y)
logreg = LogisticRegression(max_iter=1000)
logreg.fit(X_train, y_train)
ConfusionMatrixDisplay.from_estimator(logreg, X_test, y_test, display_labels=["filtered", "filtered"])
...
ConfusionMatrixDisplay.from_estimator(logreg_tfidf, X_test, y_test, display_labels=["tfidf", "tfidf"])
...
ConfusionMatrixDisplay.from_estimator(
    classifier1, unseen_bow1, unseen_bow_targets, display_labels=["unfiltered_real", "unfiltered_fake"])
ConfusionMatrixDisplay.from_estimator(
    classifier2, unseen_bow2, unseen_bow_targets, display_labels=["filtered_real", "filtered_fake"])
ConfusionMatrixDisplay.from_estimator(
    classifier3, unseen_bow3, unseen_bow_targets,  display_labels=["tfidf_real", "tfidf_fake"])
plt.show()